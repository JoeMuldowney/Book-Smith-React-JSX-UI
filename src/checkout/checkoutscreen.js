...
const getCheckOut = async () => {
  ...
  const shipresponse = await axios.get(`http://18.218.222.138:8020/shipping`, {
    params: { user: userId}})                 
    setAddress(shipresponse.data)         
    setLoading(false)
 
  const billresponse = await axios.get(`http://18.218.222.138:8020/billing`,{
    params: { user: userId}})                 
    setCard(billresponse.data)
    setLoading(false)

   // Fetch book data based on the bookId
   const cartresponse = await axios.get(`http://18.218.222.138:8020/checkout`,{
     params: { user: userId}})
