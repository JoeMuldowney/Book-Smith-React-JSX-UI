    ...
    useEffect(() => {
        const getBilling = async () => {
          try {
            ...
            const resp = await axios.get('http://localhost:8080/billing', {withCredentials: true });
            const response = await axios.get('http://localhost:8080/allcard', {withCredentials: true });
      ...
      const setCardClick = (id) => {          
          ...
          axios.put('http://localhost:8080/updatecard',{
            "id":id,
            "pay_default": true
          })
          .then(response => {        
